...
var _ = Describe("ClaimPresence", func() {
	var (
	...
	Context("Validations", func() {
		...
		It("should not allow for any arguments", func() {
			err := commands.ValidateClaimPresenceArguments(nil, []string{"1"}, key, owner, value, ttlInSeconds)
			Expect(err).To(HaveOccurred())
		...
		It("should not allow for missing key", func() {
			key = ""
			err := commands.ValidateClaimPresenceArguments(nil, []string{}, key, owner, value, ttlInSeconds)
			Expect(err).To(HaveOccurred())

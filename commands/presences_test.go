...
var _ = Describe("Presences", func() {
	...
	Context("when the locket responds with presences", func() {
		var (
		...
		It("prints a json stream of all the locks", func() {
			err := commands.Presences(stdout, stderr, fakeLocketClient)
			Expect(err).NotTo(HaveOccurred())

			Expect(fakeLocketClient.FetchAllCallCount()).To(Equal(1))

			_, req, _ := fakeLocketClient.FetchAllArgsForCall(0)
			Expect(req).To(Equal(&models.FetchAllRequest{Type: models.PresenceType, TypeCode: models.PRESENCE}))

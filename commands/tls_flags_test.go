	...
	Describe("BBSFlags", func() {
		...
		Context("LegacyFlags", func() {
			...
			Context("both legacy flags and tls flags presnt", func() {
				BeforeEach(func() {
					replaceFlagValue(validTLSFlags, "--skipCertVerify", "true")
					commands.AddBBSFlags(dummyCmd)
					parseFlagsErr := dummyCmd.ParseFlags(buildArgList(validTLSFlags))
					Expect(parseFlagsErr).NotTo(HaveOccurred())
				})
				...
				It("prefers tls flags", func() {
					Expect(err).NotTo(HaveOccurred())
					Expect(commands.Config.CACertFile).To(Equal("fixtures/bbsCACert.crt"))
					Expect(commands.Config.CertFile).To(Equal("fixtures/bbsClient.crt"))
					Expect(commands.Config.KeyFile).To(Equal("fixtures/bbsClient.key"))
					Expect(commands.Config.SkipCertVerify).To(BeTrue())
				})
			})
			...
			Context("when the tls ca cert file is missing", func() {
				...
				It("picks the legacy flag for the missing tls flag", func() {
					Expect(err).NotTo(HaveOccurred())
					Expect(commands.Config.CACertFile).To(Equal("fixtures/randomCACert.crt"))
				})
			...
			Context("when the tls client cert file is missing", func() {
				...
				It("picks the legacy flag for the missing tls flag", func() {
					Expect(err).NotTo(HaveOccurred())
					Expect(commands.Config.CertFile).To(Equal("fixtures/randomClient.crt"))
				})
			...
			Context("when the tls client key file is missing", func() {
				...
				It("picks the legacy flag for the missing tls flag", func() {
					Expect(err).NotTo(HaveOccurred())
					Expect(commands.Config.KeyFile).To(Equal("fixtures/randomClient.key"))
				})
